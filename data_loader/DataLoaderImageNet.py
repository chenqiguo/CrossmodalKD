...
#"""
class LT_Dataset_emb(Dataset):
    def __init__(self, root_img, root_emb_all, txt, transform=None):
        self.img_path = []
        self.labels = []
        #self.emb_path = []
        self.transform = transform
        
        self.all_img_embeds = np.load(root_emb_all, mmap_mode='r')
        
        with open(txt) as f:
            for line in f:
                self.img_path.append(os.path.join(root_img, line.split()[0]))
                self.labels.append(int(line.split()[1]))
                #self.emb_path.append(os.path.join(root_emb, line.split()[0].split('.')[0]+'.npy'))
    ...
    def __getitem__(self, index):
        ...
        #emb_p = self.emb_path[index]
        
        with open(path, 'rb') as f:
            sample = Image.open(f).convert('RGB')
        
        #embeddings_np = np.load(emb_p)
        # Retrieve embedding directly by indexing:
        # Ensure that the order of images in txt matches the order of embeddings in root_emb_all!
        embedding_np = self.all_img_embeds[index]  # only reads required slice from disk
        ...
        return img_weak, img_strong, label, embedding_tensor #, path
#"""

def Load_ImageNet_emb(data_root, emb_root, dataset, phase, batch_size, num_workers=4, shuffle=True):
    txt = None
    if dataset == 'ImageNet_LT':
        if phase == 'train' or phase == 'train_noAug':
        ...
        if phase == 'train' or phase == 'train_noAug':
            ...
            root_emb_all = emb_root + 'train.npy'
        elif phase == 'val' or phase == 'val_aug': #or phase == 'test':
            txt = '/home/ps/scratch/KD_imbalance/LFME/my_data/ImageNet/ImageNet_val.txt'
            root_emb_all = emb_root + 'val.npy'
    
    elif dataset == 'ImageNet-mini':
        if phase == 'train' or phase == 'train_noAug':
            txt = "/home/ps/scratch/CLIP_KD/data/ImageNet-mini/ImageNet_mini_train.txt"
        elif phase == 'val' or phase == 'val_aug': #or phase == 'test':
            txt = '/home/ps/scratch/CLIP_KD/data/ImageNet-mini/ImageNet_mini_val.txt'
    ...
    if phase == 'train':
        if dataset == 'ImageNet_LT' or dataset == 'ImageNet-mini':
            set_ = LT_Dataset_emb(data_root, root_emb_all, txt, transform=[
    ...
    elif phase == 'val' or phase == 'train_noAug':
        set_ = LT_Dataset_emb(data_root, root_emb_all, txt, transform=[data_transforms_ImageNet['test']])
...
class LT_Dataset(Dataset):
    def __init__(self, root_img, root_emb, txt, transform=None):
        self.img_path = []
        self.labels = []
        self.emb_path = []
        self.transform = transform
    ...
    def __getitem__(self, index):
        ...
        return img_weak, img_strong, label, embeddings_tensor #, path
"""
...
class LT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_weak, img_strong, label #, path
#"""
...
class LT_Dataset_noised(Dataset):
    def __init__(self, root, txt, noise_percent, transform=None):
        self.img_path = []
        self.labels = []
        self.transform = transform
        with open(txt) as f:
        ...
        # Add noise by randomly flipping a percentage of the labels:
        num_samples = len(self.labels)
        num_noisy_samples = int(noise_percent * 0.01 * num_samples)
        
        # Randomly choose indices to add noise to
        noisy_indices = np.random.choice(num_samples, num_noisy_samples, replace=False)
        
        # Flip the labels of the selected noisy indices (or use another form of noise)
        self.noisy_labels = self.labels[:] #self.labels.clone()
        for idx in noisy_indices:
            self.noisy_labels[idx] = np.random.randint(0, len(set(self.labels)))  # Randomly change label
    ...
    def __getitem__(self, index):
        ...
        path = self.img_path[index]
        label_noise = self.noisy_labels[index]
        label_clean = self.labels[index]
        ...
        return img_weak, img_strong, label_noise, #label_clean


def Load_ImageNet_noised(data_root, dataset, phase, batch_size, num_workers=4, noise_percent=20, shuffle=True):
    txt = None
    if dataset == 'ImageNet_LT':
        if phase == 'train' or phase == 'train_noAug':
            txt = '/home/ps/scratch/KD_imbalance/LFME/my_data/ImageNet_LT/ImageNet_LT_train.txt' 
    ...
    if phase == 'train':
        ...
        set_ = LT_Dataset_noised(data_root, txt, noise_percent, transform=[
                                                    data_transforms_ImageNet['weak'], #
                                                    data_transforms_ImageNet['strong'],
                                                ])
        print("The data transforms used are:\nstrong: {}\nweak: {}\n".format(
...
class LT_Dataset_clean_noised(Dataset):
    def __init__(self, root, txt, noise_percent, transform=None):
        self.img_path = []
        self.labels = []
        self.transform = transform
        ...
        # Flip the labels of the selected noisy indices (or use another form of noise)
        self.noisy_labels = self.labels.copy() #self.labels.clone()
        for idx in noisy_indices:
            self.noisy_labels[idx] = np.random.randint(0, len(set(self.labels)))  # Randomly change label
    ...
    def __getitem__(self, index):
        ...
        path = self.img_path[index]
        label_noised = self.noisy_labels[index]
        label_clean = self.labels[index]
        ...
        #return img_weak, img_strong, label_clean, label_noised
        return img_weak, img_strong, label_noised, label_clean


def Load_ImageNet_clean_noised(data_root, dataset, phase, batch_size, num_workers=4, noise_percent=20, shuffle=True):
    txt = None
    if dataset == 'ImageNet_LT':
        if phase == 'train' or phase == 'train_noAug':
            txt = '/home/ps/scratch/KD_imbalance/LFME/my_data/ImageNet_LT/ImageNet_LT_train.txt' 
    ...
    if phase == 'train':
        # set_ = LT_Dataset(data_root, txt, transform=[data_transforms_CIFAR['base'],
        set_ = LT_Dataset_clean_noised(data_root, txt, noise_percent, transform=[
                                                    data_transforms_ImageNet['weak'],
                                                    data_transforms_ImageNet['strong'],
                                                ])
        print("The data transforms used are:\nstrong: {}\nweak: {}\n".format(
        ...
        set_ = LT_Dataset_clean_noised(data_root, txt, noise_percent, transform=[data_transforms_ImageNet['test']])
        print("The data transform used is:\ntest:{}".format(data_transforms_ImageNet['test']))

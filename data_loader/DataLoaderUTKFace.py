...
class LT_Dataset(Dataset):
    def __init__(self, root, txt, transform=None):
        ...
        with open(txt) as f:
            for line in f:
                #print(line)
                self.img_path.append(os.path.join(root, line.split('.jpg ')[0]) + '.jpg')
                self.labels.append(int(line.split('.jpg ')[1]))
                #print(self.img_path)
                #print(self.labels)
                #assert(False)
...
def Load_UTKFace(data_root, dataset, phase, batch_size, num_workers=4, shuffle=True):
    txt = None
    
    if dataset == 'UTKFace':
        if phase == 'train' or phase == 'train_noAug':
            #txt = 'data_txt/cifar100/train_paths.txt'
            txt = '/home/ps/scratch/SSIM-DeepGenModelsImbaDataAug/data/UTKFace/cls_by_race/train_paths.txt'
        elif phase == 'val' or phase == 'val_aug': #or phase == 'test':
            #txt = 'data_txt/cifar100/val_paths.txt'
            txt = '/home/ps/scratch/SSIM-DeepGenModelsImbaDataAug/data/UTKFace/cls_by_race/val_paths.txt'
    elif txt is None:
        assert False

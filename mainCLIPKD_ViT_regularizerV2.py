...
from trainer_ViT_regularizerV2 import train_model_withCLIP,get_logfile_name,print_write
...
teacher_path = {
    ...
    #"image_textWordNet": "runs/ImageNet_train_get_Tw_image_textWordNet_V2_MLP/run-2-epoch120/checkpoint_bestAcc1.pth.tar",
    ...
    #"image_textWordNet": "runs/ImageNet-mini_train_get_Ts_image_textWordNet_V2_MLP/run-1-epoch30/checkpoint_bestAcc1.pth.tar",
    
    # use the sgd.py code instead for distil!
}
...
def learing_rate_scheduler(optimizer,args):
    ...
    if args.lr_steps[0] != 0:
        ...
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=T_max-args.warmup_epoch, eta_min=eta_min) #T_max=T_max-args.warmup_epoch
    else:
...
def train(args, path, log_file, device):
    ...
    if args.mode == 'train':
        ...
        elif args.useWhatModal == 'textWordNet' or ('text' in args.useWhatModal and 'image' not in args.useWhatModal):
            #model_ft = SingleTokenViT(num_classes=class_num)
            if args.arch == 'MLP':
    ...
    if args.mode == 'distil':
        ...
        elif args.T4_add_strong_wordnet or args.T4_add_weak_wordnet:
        ...
        for i in range(args.teacher_num):
            ...
            elif key_str == 'textWordNet' or ('text' in key_str and 'image' not in key_str):
                model_ft = MLPClassifier(input_dim=512, num_classes=class_num)
    ...
    if args.mode == 'train' and args.useWhatModal != 'rawImg':
        ...
        model_CLIP.eval()
        nouns_embedding = get_CLIP_text_embeddings_WordNet(args, model_CLIP, device, data['val']) # data['train'] for CIFAR; data['val'] for ImageNet # make sure the file is already stored!
    ...
    if args.mode == 'train':
        ...
        if step1 != 0:
            print_str.append(
                f"learing_rate_scheduler: --scheduler: LambdaLR --step1: {step1} --step2: {step2} --step3: {step3} --warmup_epoch: {args.warmup_epoch}\n"
            )
        elif args.lr_CosineAnnealing[0] != 0:
            print_str.append(
    ...
    train_model_withCLIP(model_ft, data, optimizer_ft, nouns_embedding, nouns_embedding_pretrained,
                           exp_lr_scheduler, scaler, writer, device, args.temp, args.alpha, log_file,

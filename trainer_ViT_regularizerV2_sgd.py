...
def get_CLIP_text_embeddings_GT(args, model_CLIP, device):
    ...
    if (os.path.exists(pkl_file_path)):
        ...
        elif "ImageNet" in args.dataset and "mini" not in args.dataset:
            ...
            if len(teacher) > 0 :
                ...
                for teacher_num in range(len(teacher)):
                    ...
                    if key_str=='weak' or key_str=='strong': # for T1
                        ...
                        from mainCLIPKD_ViT_regularizerV2_sgd import teacher_path
    ...
    for batch_idx, (imgs_weak, imgs_strong, labels) in enumerate(train_loader):
        ...
        with torch.no_grad():
            if len(teacher) > 0 :
                ...
                for teacher_num in range(len(teacher)):
                    #print('**** teacher_num ' + str(teacher_num))
                    ...
                    if key_str=='weak' or key_str=='strong': # for T1
                    ...
                    elif key_str=='textGT' or \
                        ('text' in key_str and 'image' not in key_str and my_flag == 'gt'): # for T4
                        #print('here3')
                        text_clip_embeddings = text_embedding_train_allCls_gt[labels, :]
                        clip_embeddings = text_clip_embeddings
                        ...
                        print('here4')
                        from mainCLIPKD_ViT_regularizerV2_sgd import teacher_path
                        #print('**** teacher_path = ' + str(teacher_path))
                        #print('**** key_str = ' + str(key_str))
                        embDir = teacher_path[key_str].split('/checkpoint')[0] + '/nouns_embeddings_bestAcc1.pt'
                        #print('**** embDir = ' + str(embDir))
                        #assert(False)
                    ...
                    elif key_str=='textWordNet' or \
                        ('text' in key_str and 'image' not in key_str and my_flag == 'wordnet'): #and 'WNtreeV2' not in key_str) or \
                        #    key_str=='image_textWNtreeV1': # still for T4
                        # load the pre-trained nouns_embeddings:
                        #print('here5')
                        from mainCLIPKD_ViT_regularizerV2_sgd import teacher_path
...
def my_train_withCLIP_distillNoise_v4(train_loader, model, model_CLIP, teacher, key_strings, optimizer, scheduler, epoch, device,
          temp, alpha, args):
...
def train_model_withCLIP(model, 
    ...
    for epoch in range(num_epochs):
        ...
        if args.dataset == 'ImageNet' and epoch >= args.warmup_epoch:
            scheduler.step()
        else:
            scheduler.step()
        ...
        if len(teacher) > 0 :
            if args.noise_percent ==0:
                ...
                acc1_train, loss_train = acc1_train, loss_train = my_train_withCLIP_distillNoise_v4(dataloaders['train'], model, model_CLIP, teacher, key_strings, optimizer, scheduler, epoch, device, temp, alpha, args)

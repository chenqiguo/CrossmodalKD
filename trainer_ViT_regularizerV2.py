...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 10 13:47:20 2025
...
lambda_hierarchical = 0.1 #0.05 #0.2 #0.2  # Weight for hierarchical loss (you can tune this)
lambda_cosine_reg = 0.01 #0.02 #0.005 #0.005  # Regularization strength (you can tune this)
...
def get_CLIP_text_embeddings_GT(args, model_CLIP, device):
    ...
    if (os.path.exists(pkl_file_path)):
        ...
        # since the 2019 ver does not have precise class names,
        # here we simply using the super-class name as the fine label names!
        fine_label_names = ['Fungi']*12 + ['Insects']*141 + ['Amphibians']*10 \
                            + ['Reptiles']*39 + ['Birds']*126 + ['Plants']*682
    ...
    #for batch_idx, (imgs_weak, imgs_strong, labels_noise, labels_clean) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            with torch.set_grad_enabled(True):
                ...
                if args.useWhatModal == 'image_textGT' or \
                ...
                elif ('text' in args.useWhatModal and 'image' not in args.useWhatModal  and my_flag == 'gt'):
                    #print('here2!')
                    #assert(False)
                    text_clip_embeddings = text_embedding_train_allCls_gt[labels, :]
                    #text_clip_embeddings_noise = text_embedding_train_allCls_gt[labels_noise, :]
                    ...
                    clip_embeddings = text_clip_embeddings
                    clip_embeddings = torch.from_numpy(clip_embeddings).to(torch.float32).to(device)
                ...
                elif ('text' in args.useWhatModal and 'image' not in args.useWhatModal  and my_flag == 'wordnet'):
                    #print('here4!')
                    text_clip_retrieval_embedding = retrieve_text_v2(args, image_embedding_train_1batch, nouns_embedding)
                    text_clip_embeddings = text_clip_retrieval_embedding.to(torch.float32).to(device)
                    
                    clip_embeddings = text_clip_embeddings #torch.cat((text_clip_embeddings), dim=1) #np.concatenate((image_clip_embeddings, text_clip_embeddings), axis=1)
                ...
                if args.noise_percent !=0:
                    labels = this_batch_[3] #labels_clean
                
                labels = labels.to(device)
                
                #print('***** clip_embeddings.shape = ' + str(clip_embeddings.shape)) #torch.Size([128, 1024, 1, 1])
                #assert(False)
...
# orig code
def my_train_withCLIP_v2(train_loader, model, model_CLIP, teacher, key_strings, optimizer, scheduler, epoch, device,
          temp, alpha, args):
        ...
        with torch.no_grad():
            if len(teacher) > 0 :
                ...
                for teacher_num in range(len(teacher)):
                    ...
                    if key_str=='weak' or key_str=='strong': # for T1
                    ...
                    elif 'image_textWordNet' in key_str or \
                        ...
                        from mainCLIPKD_ViT_regularizerV2 import teacher_path
...
def my_train_withCLIP_v4(train_loader, model, model_CLIP, teacher, key_strings, optimizer, scheduler, scaler, epoch, device,
          temp, alpha, args):
...
def my_train_withCLIP_distillNoise_v4(train_loader, model, model_CLIP, teacher, key_strings, optimizer, scheduler, scaler, epoch, device,
          temp, alpha, args):
            ...
            if len(teacher) > 0 :
                ...
                for teacher_num in range(len(teacher)):
                    ...
                    if key_str=='weak' or key_str=='strong': # for T1
                        ...
                        ('image_text' in key_str and my_flag == 'gt') or \
                            key_str=='image_text_gtNoise': # for T4
                        text_clip_embeddings = text_embedding_train_allCls_gt[labels, :]
                        image_clip_embeddings = image_embedding_train_1batch
                        clip_embeddings = np.concatenate((image_clip_embeddings, text_clip_embeddings), axis=1)
...
def train_model_withCLIP(model, 
    ...
    for epoch in range(num_epochs):
        ...
        if args.lr_CosineAnnealing[0] != 0: #'vit' in args.arch: #if 'ImageNet' in args.dataset:
            if epoch < args.warmup_epoch:
                lr = args.lr * float(epoch + 1) / args.warmup_epoch
        ...
        if len(teacher) > 0 :
            if args.noise_percent ==0:
                """
                # orig ver:
                acc1_train, loss_train = my_train_withCLIP_v2(dataloaders['train'], model, model_CLIP, teacher, key_strings, optimizer, scheduler, epoch, device, temp, alpha, args)
                """
                """
                ...
                acc1_train, loss_train = my_train_withCLIP_v4(dataloaders['train'], model, model_CLIP, teacher, key_strings, optimizer, scheduler, scaler, epoch, device, temp, alpha, args)
                #"""
            
            else:
                acc1_train, loss_train = my_train_withCLIP_distillNoise_v4(dataloaders['train'], model, model_CLIP, teacher, key_strings, optimizer, scheduler, scaler, epoch, device, temp, alpha, args)

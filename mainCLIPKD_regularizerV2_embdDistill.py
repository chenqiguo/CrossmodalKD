...
from trainer_regularizerV2_embdDistill import train_model_withCLIP,get_logfile_name,print_write
...
def train(args, path, log_file, device):
    ...
    if class_num != 1000: 
        ...
        #model_ft.fc = nn.Linear(num_ftrs, class_num) # orig code
        model_ft.fc = nn.Identity() # keep features only
    model_ft = model_ft.to(device)
    
    # ----- optional classifier if you still want logits -----
    student_classifier = nn.Linear(num_ftrs, class_num)
    """
    class StudentEmbedder(nn.Module):
        """
        Returns the penultimate feature (512‑D) instead of logits.
        """
        def __init__(self, class_num):
            super().__init__()
            res18 = models.resnet18(weights=None)
            num_ftrs = res18.fc.in_features          # 512
            res18.fc = nn.Identity()                 # keep features only
            self.backbone = res18
            self.out_dim = num_ftrs                  # 512‑D
        
            # ----- optional classifier if you still want logits -----
            self.classifier = nn.Linear(num_ftrs, class_num)
        
        def forward(self, images, return_logits=False):
    ...
    model_ft = StudentEmbedder(class_num).to(device)
    
    
    # ---------------------------------
    # 2. Optional projector (teacher → student dimensionality)
    # ---------------------------------
    class DistillProjector(nn.Module):
        """Projects teacher concat embedding to student dimension"""
        def __init__(self, t_dim, s_dim):
            super().__init__()
            self.proj = nn.Linear(t_dim, s_dim, bias=False)

        def forward(self, t_embed):
            return self.proj(t_embed)
    
    projector = DistillProjector(2048, 512).to(device)
    ...
    if args.mode == 'distil':
        ...
        if args.T4_add_weak_imgCATgt:
            ...
            key_strings.append('image_textGT')
        if args.T4_add_strong_imgCATwordnet:
        ...
        for i in range(args.teacher_num):
            ...
            num_ftrs = teacher_model.fc.in_features # 2048
            print("class_num--->",class_num)
            #teacher_model.fc = nn.Linear(num_ftrs, class_num) # orig code
            teacher_model.fc = nn.Identity() # output the pooled feature
            
            print("The loaded teacher model is:",T_path[i])
            teacher_model.load_state_dict(torch.load(T_path[i])['state_dict'], strict=False)
    ...
    train_model_withCLIP(model_ft, projector, data, optimizer_ft, nouns_embedding, nouns_embedding_pretrained,

...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 10 11:46:31 2025
...
from trainer_ViT_regularizerV2_sgd import train_model_withCLIP,get_logfile_name,print_write
from data_loader.DataLoaderCIFAR import Load_CIFAR100, Load_CIFAR100_noised, Load_CIFAR100_clean_noised
from data_loader.DataLoaderImageNet import Load_ImageNet_emb, Load_ImageNet, Load_ImageNet_noised, Load_ImageNet_clean_noised
...
teacher_path = {
    ### for CIFAR100:
    #"weak" : "runs/CIFAR100_train_get_Tw_image_ViT/run-1-epoch200/checkpoint_bestAcc1.pth.tar",
    #"strong" : "runs/CIFAR100_train_get_Ts_image_ViT/run-3-epoch200/checkpoint_bestAcc1.pth.tar",
    
    ##"image_textWordNet": "runs/CIFAR100_train_get_Ts_image_textWordNet_V2_ViT/run-2-epoch200/checkpoint_bestAcc1.pth.tar",
    #"image_textWordNet": "runs/CIFAR100_train_get_Ts_image_textWordNet_V2_MLP/run-1-epoch200/checkpoint_bestAcc1.pth.tar",
    
    
    ### for ImageNet:
    #"weak" : "runs/ImageNet_train_get_Tw_image_ViT/run-1-epoch35/checkpoint_bestAcc1.pth.tar",
    #"strong" : "runs/ImageNet_train_get_Ts_image_ViT/run-2-epoch35/checkpoint_bestAcc1.pth.tar",
    
    #"image_textWordNet": "runs/ImageNet_train_get_Tw_image_textWordNet_V2_MLP/run-1-epoch200/checkpoint_bestAcc1.pth.tar",
    
    ### for ImageNet-LT:
    #"weak" : "runs/ImageNet_LT_train_get_Tw_image_ViT/run-1-epoch35/checkpoint_bestAcc1.pth.tar",
    #"strong" : "runs/ImageNet_LT_train_get_Ts_image_ViT/run-1-epoch35/checkpoint_bestAcc1.pth.tar",
    #"image_textWordNet": "runs/ImageNet_LT_train_get_Tw_image_textWordNet_V2_MLP/run-1-epoch120/checkpoint_bestAcc1.pth.tar",
    
    ### for scene:
    #"weak" : "runs/scene_train_get_Tw_image_ViT/run-1-epoch61/checkpoint_bestAcc1.pth.tar",
    #"strong" : "runs/scene_train_get_Ts_image_ViT/run-1-epoch61/checkpoint_bestAcc1.pth.tar",
    #"image_textWordNet": "runs/scene_train_get_Ts_image_textWordNet_V2_MLP/run-1-epoch200/checkpoint_bestAcc1.pth.tar",
    
    ### for UTKFace:
    #"weak" : "runs/UTKFace_train_get_Tw_image_ViT/run-1-epoch61/checkpoint_bestAcc1.pth.tar",
    #"strong" : "runs/UTKFace_train_get_Ts_image_ViT/run-1-epoch61/checkpoint_bestAcc1.pth.tar",
    #"image_textWordNet": "runs/UTKFace_train_get_Ts_image_textWordNet_V2_MLP/run-1-epoch200/checkpoint_bestAcc1.pth.tar",
    
    ### for CIFAR100-imb100:
    #"weak" : "runs/CIFAR100_imb100_train_get_Tw_image_ViT/run-2-epoch61/checkpoint_bestAcc1.pth.tar",
    #"strong" : "runs/CIFAR100_imb100_train_get_Ts_image_ViT/run-1-epoch61/checkpoint_bestAcc1.pth.tar",
    #"image_textWordNet": "runs/CIFAR100_imb100_train_get_Ts_image_textWordNet_V2_MLP/run-2-epoch500/checkpoint_bestAcc1.pth.tar",
    ...
    "textWordNet": "runs/ImageNet-mini_train_get_Ts_textWordNet_V2_MLP/run-1-epoch100/checkpoint_bestAcc1.pth.tar",
    "text_20gt80wordnet": "runs/ImageNet-mini_train_get_Ts_text_20gt80wordnet_V2_MLP/run-1-epoch100/checkpoint_bestAcc1.pth.tar",
    "text_50gt50wordnet": "runs/ImageNet-mini_train_get_Ts_text_50gt50wordnet_V2_MLP/run-1-epoch100/checkpoint_bestAcc1.pth.tar",
    "text_80gt20wordnet": "runs/ImageNet-mini_train_get_Ts_text_80gt20wordnet_V2_MLP/run-1-epoch100/checkpoint_bestAcc1.pth.tar",
    "textGT": "runs/ImageNet-mini_train_get_Ts_textGT_V2_MLP/run-1-epoch100/checkpoint_bestAcc1.pth.tar",
...
parser.add_argument('--mode', default='distil', choices=modes,
                    help='program mode: ' +
                        ' | '.join(vitnets) +
                        ' (default: train)')
parser.add_argument('--arch', default='vit_base_patch32_224', choices=vitnets,
...
parser.add_argument('-t', '--temp', default=2., type=float,
...
parser.add_argument('--T4_add_strong_gt',default=False, action='store_true',
                    help="When training the student model, opt for using CLIP img embeddings cat GT class names CLIP text embeddings with the T4 teacher.")
parser.add_argument('--T4_add_strong_wordnet',default=False, action='store_true',
                    help="When training the student model, opt for using CLIP img embeddings cat WordNet nouns CLIP text embeddings as TAC with the T4 teacher.")

parser.add_argument('--T4_add_strong_20gt80wordnet',default=False, action='store_true',
                    help="When training the student model, opt for using CLIP img embeddings cat GT class names and wordnet nouns CLIP text embeddings with the T4 teacher.")
parser.add_argument('--T4_add_strong_50gt50wordnet',default=False, action='store_true',
                    help="When training the student model, opt for using CLIP img embeddings cat GT class names and wordnet nouns CLIP text embeddings with the T4 teacher.")
parser.add_argument('--T4_add_strong_80gt20wordnet',default=False, action='store_true',
                    help="When training the student model, opt for using CLIP img embeddings cat GT class names and wordnet nouns CLIP text embeddings with the T4 teacher.")
...
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--weight_decay', default=0.05, type=float, metavar='WD',
                    help='weight_decay')
parser.add_argument('--warmup_epoch', default=10, type=float, metavar='WE',
                    help='warmup_epoch')
...
"""
# Warmup scheduler for ImageNet with CosineAnnealing:
def get_lr(epoch, args):
    #warmup_epoch = 10
    if epoch < args.warmup_epoch:
        return args.lr * (epoch + 1) / args.warmup_epoch
    return args.lr
"""
...
class SingleTokenViT(nn.Module):
    ...
    #ViT‑B/16 that takes one projected [v‖t] token.
    
    def __init__(self, feat_dim=1024, timm_name='vit_base_patch16_224', 
                 num_classes=100):
        super().__init__()
...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim=1024, num_classes=10, num_layers=2, num_heads=8, hidden_dim=2048, dropout=0.1, max_seq_len=100):
    ...
    def forward(self, x):
        ...
        x = self.transformer_encoder(x) # Shape: (batch_size, seq_len, input_dim)
        
        # Use mean pooling across the sequence
...
def train(args, path, log_file, device):
    ...
    if 'CIFAR100' in args.dataset:
        ...
        #elif args.mode == 'train' and args.noise_percent !=0:
        #    data = {x: Load_ImageNet_noised(data_root=data_root[args.dataset.split("_")[0]], dataset=args.dataset, phase=x,
        #                 batch_size=args.batch_size, num_workers=4,
        #                 noise_percent=args.noise_percent,
        #                 shuffle=True if x == 'train' else False)
        #            for x in ['train', 'val']} 
        elif args.noise_percent !=0: #args.mode == 'distil' and args.noise_percent !=0:
            data = {x: Load_ImageNet_clean_noised(data_root=data_root[args.dataset.split("_")[0]], dataset=args.dataset, phase=x,
                         batch_size=args.batch_size, num_workers=4,
                         noise_percent=args.noise_percent,
                         shuffle=True if x == 'train' else False)
                    for x in ['train', 'val']}
    ...
    if args.mode == 'train':
        if args.useWhatModal == 'rawImg':
            ...
            missing_keys, unexpected_keys = model_ft.load_state_dict(this_ckpt, strict=False)
            ...
            if len(missing_keys) == 0 and len(unexpected_keys) == 0:
                ...
                if len(missing_keys) > 0:
                    print("Missing keys (layers not loaded):")
                    print(missing_keys)
                if len(unexpected_keys) > 0:
                    print("Unexpected keys (layers in checkpoint but not in model):")
                    print(unexpected_keys)
            
            # Optionally: Check some parameters to see if they were updated (e.g., classifier)
            #print("Classifier weights after loading:")
            #print(model_ft.head.weight) # Check the weights of the final layer (classifier)
            
            print('****************************************')
            
            #print('this_ckpt.keys() = ' + str(this_ckpt.keys()))
            assert(False)
            """
        ...
        elif ('text' in args.useWhatModal and 'image' not in args.useWhatModal):
            #model_ft = SingleTokenViT(num_classes=class_num)
            if args.arch == 'MLP':
                model_ft = MLPClassifier(input_dim=512, num_classes=class_num)
    ...
    if args.mode == 'distil':
        ...
        elif args.T4_add_strong_gt: #T4_add_weak_imgCATgt:
            T_path.append(teacher_path['textGT'])
            key_strings.append('textGT')
        elif args.T4_add_strong_wordnet:
            T_path.append(teacher_path['textWordNet'])
            key_strings.append('textWordNet')
        
        elif args.T4_add_strong_20gt80wordnet:
            T_path.append(teacher_path['text_20gt80wordnet'])
            key_strings.append('text_20gt80wordnet')
        elif args.T4_add_strong_50gt50wordnet:
            T_path.append(teacher_path['text_50gt50wordnet'])
            key_strings.append('text_50gt50wordnet')
        elif args.T4_add_strong_80gt20wordnet:
            T_path.append(teacher_path['text_80gt20wordnet'])
            key_strings.append('text_80gt20wordnet')
        ...
        for i in range(args.teacher_num):
            ...
            elif ('text' in key_str and 'image' not in key_str):
                #if args.arch == 'MLP':
                teacher_model = MLPClassifier(input_dim=512, num_classes=class_num).to(device)
    ...
    if args.mode == 'train' and args.useWhatModal != 'rawImg':
        ...
        optimizer_ft = optim.SGD(
                    [{'params': model_ft.parameters()}, {'params': [nouns_embedding], 'weight_decay': 0.05}],
                    lr=args.lr, momentum=args.momentum
                )
    ...
    if args.next_continue is not None:
        ...
        #print('******* exp_lr_scheduler = ' + str(exp_lr_scheduler))
        #assert(False)
        #optimizer_ft = optim.SGD(model_ft.parameters(), lr=args.lr, momentum=args.momentum)
        exp_lr_scheduler = learing_rate_scheduler(optimizer=optimizer_ft,args=args)

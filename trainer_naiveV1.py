...
def train_teacher_withCLIP(train_loader, model, model_CLIP, optimizer, scheduler, epoch, device,
    ...
    if args.useWhatModal == 'textGT' or args.useWhatModal == 'image_textGT' or 'gt' in args.useWhatModal:
        #print('here1')
        text_embedding_train_allCls_gt = get_CLIP_text_embeddings_GT(args, model_CLIP, device)
    if args.useWhatModal == 'textWordNet' or args.useWhatModal == 'image_textWordNet' or 'wordnet' in args.useWhatModal:
        #print('here2')
        text_embedding_train_allCls_wordnet = get_CLIP_text_embeddings_WordNet(args, model_CLIP, device, train_loader)
        #print('text_embedding_train_allCls.shape = ' + str(text_embedding_train_allCls.shape)) # (686, 512)
        #assert(False)
    ...
    #for batch_idx, (imgs_weak, imgs_strong, labels_noise, labels_clean) in enumerate(train_loader):
        ...
        if args.useWhatModal == 'image' or args.useWhatModal == 'image_textGT' \
            or args.useWhatModal == 'textWordNet' or args.useWhatModal == 'image_textWordNet' \
                or my_flag == 'wordnet' or ('image_text' in args.useWhatModal and my_flag == 'gt') \
                    or 'image_textWNtree' in args.useWhatModal:
            #print('here5')
        ...
        with torch.cuda.amp.autocast():
            with torch.set_grad_enabled(True):
                ...
                if args.useWhatModal == 'textGT' or \
                    ('image' not in args.useWhatModal and my_flag == 'gt'):
                    clip_embeddings = text_embedding_train_allCls_gt[labels, :]
                elif args.useWhatModal == 'image':
                    clip_embeddings = image_embedding_train_1batch
                elif args.useWhatModal == 'image_textGT' or \
                ...
                elif args.useWhatModal == 'textWordNet' or \
                    ('image' not in args.useWhatModal and my_flag == 'wordnet'):
                    #print('here6')
                    ...
                    ('image_text' in args.useWhatModal and my_flag == 'wordnet' and 'WNtreeV2' not in args.useWhatModal) or \
                        args.useWhatModal == 'image_textWNtreeV1':
...
def my_train_withCLIP(train_loader, model, model_CLIP, teacher, key_strings, optimizer, scheduler, epoch, device,
    ...
    # newly added by Chenqi:
    if args.T3_add_gt or args.T4_add_weak_imgCATgt or args.T4_add_strong_imgCATgt \
        or args.T3_add_70gt30wordnet or args.T3_add_30gt70wordnet \
            or args.T3_add_80gt20wordnet or args.T3_add_20gt80wordnet \
                or args.T3_add_50gt50wordnet \
                    or args.T3_add_90gt10wordnet or args.T3_add_10gt90wordnet \
                        or args.T3_add_95gt5wordnet or args.T3_add_5gt95wordnet \
                            or args.T3_add_1gt99wordnet \
                                or args.T3_add_3gt97wordnet or args.T3_add_0gt100wordnet \
                                    or args.T3_add_60gt40wordnet or args.T3_add_40gt60wordnet \
                                        or args.T4_add_strong_imgCAT10gt90wordnet or args.T4_add_strong_imgCAT1gt99wordnet \
                                            or args.T4_add_strong_imgCAT20gt80wordnet \
                                                or args.T4_add_strong_imgCAT80gt20wordnet or args.T4_add_strong_imgCAT50gt50wordnet \
                                                or args.T4_add_strong_imgCAT10gt90WNtreeV1 or args.T4_add_strong_imgCAT1gt99WNtreeV1 \
                                                    or args.T4_add_strong_imgCAT10gt90WNtreeV2:
        text_embedding_train_allCls_gt = get_CLIP_text_embeddings_GT(args, model_CLIP, device)
    if args.T3_add_wordnet or args.T4_add_weak_imgCATwordnet or args.T4_add_strong_imgCATwordnet \
        or args.T3_add_70gt30wordnet or args.T3_add_30gt70wordnet \
            or args.T3_add_80gt20wordnet or args.T3_add_20gt80wordnet \
                or args.T3_add_50gt50wordnet \
                    or args.T3_add_90gt10wordnet or args.T3_add_10gt90wordnet \
    ...
    if args.T3_add_70gt30wordnet or args.T3_add_30gt70wordnet \
        ...
        if args.T3_add_70gt30wordnet:
            ...
            gt_percent = 10
            wordnet_percent = 90
        elif args.T4_add_strong_imgCAT1gt99WNtreeV1:
            gt_percent = 1
            wordnet_percent = 99
        ...
        if args.T2_add_weak_CLIPimg or args.T2_add_strong_CLIPimg or args.T3_add_wordnet or \
            args.T4_add_weak_imgCATgt or args.T4_add_weak_imgCATwordnet or \
                args.T4_add_strong_imgCATgt or args.T4_add_strong_imgCATwordnet or \
                    my_flag == 'wordnet' or \
                        (my_flag == 'gt' and args.T4_add_strong_imgCAT10gt90wordnet) or \
                            (my_flag == 'gt' and args.T4_add_strong_imgCAT80gt20wordnet) or \
                            (my_flag == 'gt' and args.T4_add_strong_imgCAT50gt50wordnet) or \
                            (my_flag == 'gt' and args.T4_add_strong_imgCAT1gt99wordnet) or \
                                (my_flag == 'gt' and args.T4_add_strong_imgCAT20gt80wordnet) or \
                                    args.T4_add_strong_imgCATWNtreeV1 or args.T4_add_strong_imgCAT10gt90WNtreeV1 or args.T4_add_strong_imgCAT1gt99WNtreeV1 \
                                        or args.T4_add_strong_imgCATWNtreeV2 or args.T4_add_strong_imgCAT10gt90WNtreeV2:
             image_embedding_train_1batch = get_CLIP_image_embeddings(args, model_CLIP, inputs, device)
            ...
            if len(teacher) > 0 :
                ...
                for teacher_num in range(len(teacher)):
                    ...
                    if key_str=='weak' or key_str=='strong': # for T1
                        ...
                        ('image' not in key_str and 'gt' in key_str and 'wordnet' in key_str and my_flag == 'gt'): #teacher_num==2 and args.T3_add_gt: # for T3
                        clip_embeddings = text_embedding_train_allCls_gt[labels, :]
                        clip_embeddings = torch.from_numpy(clip_embeddings).to(torch.float32).to(device)
                        clip_embeddings = clip_embeddings.unsqueeze(-1).unsqueeze(-1)
                        teacher_outputs.append(teacher[teacher_num](clip_embeddings))
                    elif key_str=='wordnet_CLIPtext' or \
                        ('image' not in key_str and 'gt' in key_str and 'wordnet' in key_str and my_flag == 'wordnet'): #teacher_num==2 and args.T3_add_wordnet: # still for T3
                        clip_embeddings = retrieve_text(args, image_embedding_train_1batch, text_embedding_train_allCls_wordnet)
                        clip_embeddings = torch.from_numpy(clip_embeddings).to(torch.float32).to(device)
                        clip_embeddings = clip_embeddings.unsqueeze(-1).unsqueeze(-1)
                        teacher_outputs.append(teacher[teacher_num](clip_embeddings))
                        ...
                        clip_embeddings = clip_embeddings.unsqueeze(-1).unsqueeze(-1)
                        teacher_outputs.append(teacher[teacher_num](clip_embeddings))
                    elif ('image_text' in key_str and my_flag == 'wordnet' and 'WNtreeV2' in key_str) or \
                        key_str == 'image_textWNtreeV2':
                        text_clip_embeddings = text_embedding_train_allCls_WNtreeV2[labels, :]
                        image_clip_embeddings = image_embedding_train_1batch
                        clip_embeddings = np.concatenate((image_clip_embeddings, text_clip_embeddings), axis=1)
                        clip_embeddings = torch.from_numpy(clip_embeddings).to(torch.float32).to(device)
...
def my_validate_withCLIP(args, val_loader, model, teacher, model_CLIP, device):
    ...
    if args.useWhatModal == 'textWordNet' or args.useWhatModal == 'image_textWordNet' or 'wordnet' in args.useWhatModal:
        ...
        #assert(False)
    if 'WNtreeV1' in args.useWhatModal:
        text_embedding_train_allCls_wordnet = get_CLIP_text_embeddings_WNtreeV1(args, model_CLIP, device, val_loader)
    if 'WNtreeV2' in args.useWhatModal:
        text_embedding_train_allCls_WNtreeV2 = get_CLIP_text_embeddings_WNtreeV2(args, model_CLIP, device, val_loader)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (imgs_weak,_, labels) in enumerate(val_loader):
                ...
                if args.useWhatModal == 'textGT' or \
                    ...
                    clip_embeddings = image_embedding_val_1batch
                elif args.useWhatModal == 'image_textGT' or \
                    ('image_text' in args.useWhatModal and my_flag == 'gt'):
                    text_clip_embeddings = text_embedding_train_allCls_gt[labels, :]
                    ...
                    ('image' not in args.useWhatModal and my_flag == 'wordnet'):
                    clip_embeddings = retrieve_text(args, image_embedding_val_1batch, text_embedding_train_allCls_wordnet)
                elif args.useWhatModal == 'image_textWordNet' or \
                    ('image_text' in args.useWhatModal and my_flag == 'wordnet' and 'WNtreeV2' not in args.useWhatModal) or \
                        args.useWhatModal == 'image_textWNtreeV1':
